...
export BASEROW_INBOUND_EMAIL_RECEIVER_URL="${BASEROW_INBOUND_EMAIL_RECEIVER_URL:-http://localhost:${BASEROW_INBOUND_EMAIL_WEBAPI_PORT:-8880}}"
# The backend runs in this container, so the receiver's web API only needs to
# listen on loopback.
export BASEROW_INBOUND_EMAIL_WEBAPI_BIND_IP="${BASEROW_INBOUND_EMAIL_WEBAPI_BIND_IP:-127.0.0.1}"
...
# The previous version never runs next to this container, so encryption at rest can
# be enabled as soon as the migrations ran.
export BASEROW_ENCRYPT_DATA_ON_STARTUP=${BASEROW_ENCRYPT_DATA_ON_STARTUP:-true}
